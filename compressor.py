...
class Quantconv(nn.Module):
    def __init__(self,ddconfig,embed_dim=4):
        super().__init__()
        self.qconv=torch.nn.Conv2d(2 * ddconfig["z_channels"], 2 * embed_dim, 1)
        self.post_quant_conv=torch.nn.Conv2d(embed_dim, ddconfig["z_channels"], 1)
    def encode(self,x):
        moments = self.qconv(x)
...
Qconfigs=OmegaConf.load("./Configs/compressor/default.yaml")



def Get_compressor(configs):
    Compressor = compressor_v3(**configs['compress_base'])
    return Compressor




class Compression_from_R(CompressionModel):
    def __init__(self, K=8, N=256, M=256, num_slice=10, channel_c=[8,8,8,8,16,16,32,32,64,64], cb_size=8192, **kwargs):
        super().__init__()

        self.slice_num = num_slice
        self.slice_ch = channel_c

        self.encoder = Encoder(K, M)
        self.hyper_enc = HyperEncoder(N, M)
        self.hyper_dec = HyperDecoder(N, M)
        self.decoder = Decoder(M)
        self.out = nn.Conv2d(M, K, 3, 1, 1)
        ...
        self.entropy_parameters_anchor = nn.ModuleList(
            EntropyParametersEX(in_dim=M * 2 + channel_c[i] * 2, out_dim=channel_c[i] * 2)
            if i else EntropyParametersEX(in_dim=M * 2, out_dim=channel_c[i] * 2)
            for i in range(num_slice)
        )
        ...
        self.entropy_parameters_nonanchor = nn.ModuleList(
            EntropyParametersEX(in_dim=M * 2 + channel_c[i] * 4, out_dim=channel_c[i] * 2)
            if i else EntropyParametersEX(in_dim=M * 2 + channel_c[i] * 2, out_dim=channel_c[i] * 2)
            for i in range(num_slice)
        )
    ...
    def forward(self, x,c=None,noisequant=False,return_y=False):
        y = self.encoder(x)
        z = self.hyper_enc(y)
        ...
        y_likelihoods = []
        q_likelihoods = []
        num_bits = np.log2(self.codebook_size)*z_q.shape[-2]*z_q.shape[-1]*z_q.shape[0]
        z_likelihoods=torch.tensor(num_bits,device=z_q.device)
        ...
        return {
            ...
            "q_likelihoods": {"y": q_likelihoods},
            'codebook_loss': emb_loss,
            'z_bits': z_likelihoods,
        }
    ...
    def compress(self, x):
        ...
        for idx, y_slice in enumerate(y_slices):
            ...
            if idx == 0:
                ...
                slice_anchor = compress_anchor(self.gaussian_conditional, slice_anchor, scales_anchor, means_anchor,
                ...
                slice_nonanchor = compress_nonanchor(self.gaussian_conditional, slice_nonanchor, scales_nonanchor,
                                                     means_nonanchor, symbols_list, indexes_list)
    ...
    def decompress(self, strings, shape):
        ...
        for idx in range(self.slice_num):
            if idx == 0:
                ...
                scales_anchor, means_anchor = params_anchor.chunk(2, 1)
                # decompress anchor
                slice_anchor = decompress_anchor(self.gaussian_conditional, scales_anchor, means_anchor, decoder, cdf,
                                                 cdf_lengths, offsets)
                # Non-anchor
                # local_ctx: [B,2 * C, H, W]
                local_ctx = self.local_context[idx](slice_anchor)
                ...
                slice_nonanchor = decompress_nonanchor(self.gaussian_conditional, scales_nonanchor, means_nonanchor,
...
class compressor_v3(Compression_from_R):
    def __init__(self,*args,**kwargs):
        super().__init__(*args, **kwargs)
        self._init_quant_stage(Qconfigs)
    def _init_quant_stage(self,configs):
        quant = Quantconv(configs['model']['params']['first_stage_config']['params']['ddconfig'])
        q_dict = torch.load(".ckpt/quant_module/quant.pth")
        postq_dict = torch.load(".ckpt/quant_module/post_quant.pth")
        q_dict.update(postq_dict)
        quant.load_state_dict(q_dict)
        self.quant = quant.eval()
        for param in self.quant.parameters():
            param.requires_grad = False
    def forward(self,x,c=None,**kwargs):
        out_dict=super().forward(x,c,**kwargs)
        post=self.quant.encode(out_dict['x_hat'])
        out_dict['x_hat']=post.mode()
        return out_dict
