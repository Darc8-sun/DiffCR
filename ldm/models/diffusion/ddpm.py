...
class DDPM(pl.LightningModule, ImageLoggerMixin):
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        SNR_diff = self.alphas_cumprod / (1. - self.alphas_cumprod)
        # SNR_diff= 0.5 * np.sqrt(torch.Tensor(alphas_cumprod)) / (2. * 1 - torch.Tensor(alphas_cumprod))
        SNR_diff[0]=SNR_diff[1]
        SNR_order,SNR_index=SNR_diff.sort()
        self.register_buffer('SNR_index', SNR_index, persistent=False)
        self.register_buffer('SNR_order', SNR_order, persistent=False)
        self.register_buffer('SNR_diff', SNR_diff, persistent=False)
        self.register_buffer('lvlb_weights', lvlb_weights, persistent=False)
        assert not torch.isnan(self.lvlb_weights).all()
        assert not torch.isnan(self.SNR_diff).all()
...
class LatentDiffusion(DDPM):
    ...
    def get_super_input(self,batch, k):
        x = super().get_input(batch, k)
        return x
    # @torch.no_grad()
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None, return_x=False,cldm_1=False):
        x = super().get_input(batch, k)
        if bs is not None:
            x = x[:bs]
        x = x.to(self.device)
    ...
    def forward(self, x, c,org=None, *args, **kwargs):
        ...
        return self.p_losses(x, c, t,org=org, *args, **kwargs)
